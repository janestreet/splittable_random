...
let%expect_test "bool fairness" =
  ...
  for seed = 1 to trial_count do
    ...
    for _ = 1 to 1_000 do
      if Splittable_random.bool state then Int.incr true_count else Int.incr false_count
    done;
...
module%test [@name "int64"] _ = struct
  open Int64.O
      ...
      List.iter bounds ~f:(fun hi ->
        ...
        then
          for _ = 1 to 1_000 do
            ...
            then
              Error.raise_s
                [%message "out of bounds" (choice : int64) (lo : int64) (hi : int64)]
          done))
  ;;
    ...
    List.iter [ 1L; 10L; 100L; 1000L ] ~f:(fun range ->
      ...
      for _ = 1 to 100 do
        ...
        Array.iteri count ~f:(fun value count ->
          if Int.equal count 0
          then
            Error.raise_s
              [%message
                "failed to generate value" (value : int) (lo : int64) (hi : int64)])
      done)
  ;;
  ...
  let test_bias_of_mean ~lo ~hi ~sample_size state =
    ...
    let rec loop iters accum =
      if iters <= 0L then accum else loop (iters - 1L) (accum +. draw ())
    in
...
module%test [@name "float"] _ = struct
  let bounds =
      ...
      List.iter bounds ~f:(fun hi ->
        ...
        then
          for _ = 1 to 1000 do
            ...
            then
              Error.raise_s
                [%message "float out of bounds" (float : float) (lo : float) (hi : float)]
          done))
  ;;
